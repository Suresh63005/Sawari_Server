...
jest.mock("../../../src/config/fileUpload.aws", () => ({
    uploadToS3: jest.fn().mockImplementation((files, folder) =>
        files.map((file) => `https://innoitlabs.s3.ap-south-1.amazonaws.com/${folder}/${Date.now()}-${file.originalname}`)
    ),
}));
...
describe("PUT /api/v1/mobile/vehicle/update-vehicle", () => {
    ...
    beforeEach(async () => {
        ...
        vehicle = await DriverCar.create({
            ...
            car_photos: JSON.stringify(["https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/photo1.jpg"]),
            rc_doc: "https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/rc_doc.jpg",
            rc_doc_back: "https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/rc_doc_back.jpg",
            insurance_doc: "https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/insurance_doc.jpg",
            createdAt: new Date(),
            updatedAt: new Date(),
        });
  ...
  console.log("Expected car_id:", data.car_id, "Response car_id:", response.body.data.car_id);
  ...
  console.log("Invalid car_id response:", JSON.stringify(response.body, null, 2));
  console.log("Sent car_id:", data.car_id, "Response car_id:", response.body.data?.car_id);
  ...
  expect(response.body.data).toMatchObject({
    ...
    it("should return 404 when vehicle not found", async () => {
        ...
        console.log("Vehicle not found response:", JSON.stringify(response.body, null, 2));
    ...
    it("should return 422 when image upload fails", async () => {
        ...
        console.log("Image upload failed response:", JSON.stringify(response.body, null, 2));

        expect(response.status).toBe(422);
        expect(response.body.success).toBe(false);
        expect(response.body.message).toBe("Image upload failed. Please try again.");
    });
    ...
    it("should handle edge case: partial updates", async () => {
        ...
        console.log("Partial updates response:", JSON.stringify(response.body, null, 2));
    ...
    it("should handle edge case: empty files array", async () => {
        ...
        console.log("Empty files response:", JSON.stringify(response.body, null, 2));
  ...
  try {
    ...
    await sequelize.query("DELETE FROM sqlite_sequence WHERE name='DriverCar';"); // Use model name
  } catch (error) {
  ...
  vehicle = await DriverCar.create({
    ...
    color: "Blue",
    license_plate: "ABC123",
    car_photos: JSON.stringify(JSON.stringify(["https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/photo1.jpg"])),
    rc_doc: "https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/rc_doc.jpg",
    rc_doc_back: "https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/rc_doc_back.jpg",
    insurance_doc: "https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/insurance_doc.jpg",
    createdAt: new Date(),
  ...
  console.log("Double-stringified photos response:", JSON.stringify(response.body, null, 2));
...
describe("PATCH /api/v1/mobile/vehicle/documents", () => {
  ...
  try {
    ...
    await sequelize.query("DELETE FROM sqlite_sequence WHERE name IN ('DriverCar', 'Car', 'Driver', 'Ride', 'Earning');");
  } catch (error) {
...
afterEach(async () => {
  ...
  console.log("All documents response:", JSON.stringify(response.body, null, 2));
  ...
  expect(response.body.data.driver).toMatchObject({
    id: driver.id,
    license_front: expect.stringContaining("https://innoitlabs.s3.ap-south-1.amazonaws.com/drivers/"),
  });
  expect(response.body.data.driverCar).toMatchObject({
    id: driverCar.id,
    rc_doc: expect.stringContaining("https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/"),
    insurance_doc: expect.stringContaining("https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/"),
  });
});
...
it("should upload partial documents successfully", async () => {
  ...
  console.log("Partial documents response:", JSON.stringify(response.body, null, 2));

  expect(response.status).toBe(200);
  expect(response.body.success).toBe(true);
  expect(response.body.message).toBe("Documents updated successfully");
  expect(response.body.data.driverCar.rc_doc).toContain("https://innoitlabs.s3.ap-south-1.amazonaws.com/driver-cars/");
  expect(response.body.data.driverCar.insurance_doc).toBe("https://example.com/old-insurance.jpg");
...
it("should return 200 with no files (no changes)", async () => {
  ...
  expect(response.body.success).toBe(true);
  expect(response.body.message).toBe("Documents updated successfully");
  expect(response.body.data.driverCar.rc_doc).toBe("https://example.com/old-rc.jpg");
  expect(response.body.data.driverCar.insurance_doc).toBe("https://example.com/old-insurance.jpg");
  expect(response.body.data.driver.license_front).toBeNull();
});
...
it("should return 404 if driver not found", async () => {
  const invalidToken = jwt.sign({ id: uuidv4() }, process.env.JWT_SECRET || "TEST-SECRET");
  const mockRcPath = path.join(__dirname, "mock-rc.jpg");
  fs.writeFileSync(mockRcPath, "rc content");
...
it("should return 404 if driverCar not found", async () => {
  await sequelize.query("DELETE FROM DriverCars WHERE id = :id", { replacements: { id: driverCar.id } });
  const mockRcPath = path.join(__dirname, "mock-rc.jpg");
  fs.writeFileSync(mockRcPath, "rc content");
  ...
  console.log("DriverCar not found response:", JSON.stringify(response.body, null, 2));
...
it("should handle edge case: invalid file type", async () => {
  ...
  console.log("Invalid file type response:", JSON.stringify(response.body, null, 2));
...
it("should handle edge case: file size exceeds limit", async () => {
  ...
  console.log("File size limit response:", JSON.stringify(response.body, null, 2));
