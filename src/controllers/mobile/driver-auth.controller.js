...
const updateProfileAndCarDetails = async (req, res) => {
  try {
    ...
    for (const field of profileFields) {
      if (req.body?.[field]) { // Expecting a URL from the frontend
        const uploadedUrl = req.body[field];

        // If old file exists and is different, delete it from s3
        if (driver?.[field] && driver[field] !== uploadedUrl) {
          console.log(`Deleting old ${field} from S3:`, driver[field]);
          await deleteFromS3(driver[field]);
        }
    ...
    console.log("All profile files processed. Current uploaded file list:", req.body);
    ...
    // Step 9: Check if any car-related data is provided
    const hasCarFiles = req.body?.rc_doc || req.body?.rc_doc_back || req.body?.insurance_doc || req.body?.car_photos;
    const hasCarFields = carData.car_id || carData.license_plate || carData.color;
    const hasCarData = hasCarFields || hasCarFiles;
    ...
    if (req.body?.car_photos) {
      if (req.body.car_photos.some((url) => !url)) {
        console.log("Ignoring empty car_photos url");
      } else {
        carData.car_photos = req.body.car_photos;
        console.log("Uploaded car photos URLs:", req.body.car_photos);
      }
    }
    ...
    if (req.body?.rc_doc) {
      ...
      carData.rc_doc_status = "pending";
      hasCarVerificationFiles = true;
      console.log("Uploaded RC document URL and set rc_doc_status to 'pending':", carData.rc_doc);
    }
    if (req.body?.insurance_doc) {
      carData.insurance_doc = req.body.insurance_doc;
      carData.insurance_doc_status = "pending";
      hasCarVerificationFiles = true;
      console.log("Uploaded insurance document URL and set insurance_doc_status to 'pending':", carData.insurance_doc);
    }
