...
const createTicketByUser = async (req, res) => {
  ...
  if (!isValidPriority(normalizedPriority)) {
    return res.status(400).json({
      success: false,
      message: "Invalid priority. priority must be 'low' or 'medium' or 'high' or 'urgent' "
    });
  }
  ...
  try {
    ...
    res.status(201).json({ success: true, message: "Ticket created successfull", data: ticket });
  } catch (error) {
...
const getTicketsByUser = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({success: true, message: "Ticket fetched sucessfully", data: tickets});
  } catch (error) {
...
const ticketview = async (req, res) => {
  ...
  try {
    ...
    // res.status(200).json(tickets);
    res.status(200).json({success: true, message: "Ticket reviewed sucessfully", data: tickets});
  } catch (error) {
    console.error("Get Tickets Controller Error:", error);
    res.status(500).json({success: false, message: error.message || "Internal server error" });
  }
